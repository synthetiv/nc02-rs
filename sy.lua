...
function PercVoice:jump_to_next_step()
  local t = (self.tick + self.shift - 1) % self.pattern.length + 1
  for s, step in ipairs(self.pattern) do
    if step.t >= t then
      self.shift = step.t - self.tick
      self.next_step_index = s
      self.next_step = step
      return
    end
  end
  self.shift = 1 - self.tick
...
function PercVoice:next_tick()
  ...
  while (self.tick + self.shift - 1) % self.pattern.length + 1 == self.next_step.t do
    ...
    if trig and not did_trig then
      ...
      -- calculate repeats
      local repeats = 0
      if math.random() < step.p_repeat then
        repeats = 7
      end
  ...
  if not did_trig and self.repeats > 0 then
    self:play()
  ...
  if did_step then
    ...
    history_step.x = self.step.x
    history_step.trig = did_trig
    history_step.echo = self.echo
...
drums[1]:set_pattern{
  ...
  bd{ t = 9,    p_trig = 1,     p_shift = 0.25,  p_reset = 0.9  },
...
function redraw()
  ...
  for d = 1, 3 do
    ...
    local y = d * 64 / 4
    for t = 1, 32 do
      local pt = (t - 1) % pattern.length + 1
      local now = pt == (drum.tick + drum.shift - 1) % pattern.length + 1
      local x = t * 110 / 33
      local has_step = false
      while pattern[s].t == pt do
        screen.circle(x, y, has_step and 4 or 2)
        if now and drum.history[drum.history_index].trig then
          screen.level(15)
          screen.fill()
        else
          screen.level(now and 7 or 2)
          screen.stroke()
        end
        s = s % #pattern + 1
        has_step = true
      ...
      if not has_step then
        screen.circle(x, y, 0.8)
        screen.level(now and 7 or 1)
        screen.fill()
      end
    end
    screen.move(128, y)
    screen.level(4)
    screen.text_right(drum.shift)
    screen.stroke()
  end
  --]]
